...
describe('Binder model - database acceptance', function() {
  ...
  describe('#create', function() {
    it('should default status to 0', function(done) {
      new Binder({ title: 'B1', createdBy: 'tester' })
        .save()
        .then(function(doc) {
    ...
    it('should default archived to false', function(done) {
      new Binder({ title: 'B2', createdBy: 'tester' })
        .save()
        .then(function(doc) {
    ...
    it('should create a binder with an empty works array', function(done) {
      new Binder({ title: 'B3', createdBy: 'tester' })
        .save()
        .then(function(doc) {
    ...
    it('should default all progress fields to 0', function(done) {
      new Binder({ title: 'B4', createdBy: 'tester' })
        .save()
        .then(function(doc) {
  ...
  describe('#read', function() {
    it('should find a binder by _id', function(done) {
      new Binder({ title: 'FindMe' })
        .save()
        .then(function(saved) {
          return Binder.findById(saved._id);
        })
        .then(function(doc) {
          doc.should.exist;
    ...
    it('should find non-archived binders', function(done) {
      ...
      Promise.all(saves)
        .then(function() {
          return Binder.find({ archived: false });
        })
        .then(function(docs) {
    ...
    it('should find a binder by owner', function(done) {
      ...
      Promise.all(saves)
        .then(function() {
          return Binder.find({ owner: 'alice' });
        })
        .then(function(docs) {
          docs.should.have.lengthOf(1);
  ...
  describe('#update - status transitions', function() {
    it('should transition status from 0 to 1', function(done) {
      new Binder({ title: 'Trans1' })
        .save()
        .then(function(doc) {
          return Binder.findByIdAndUpdate(
            doc._id,
            { $set: { status: 1 } },
            { new: true }
          );
    ...
    it('should transition status from 1 to 2 (completed)', function(done) {
      new Binder({ title: 'Trans2', status: 1 })
        .save()
        .then(function(doc) {
          return Binder.findByIdAndUpdate(
            doc._id,
    ...
    it('should transition status to 3 (archived)', function(done) {
      new Binder({ title: 'Trans3', status: 2 })
        .save()
        .then(function(doc) {
          return Binder.findByIdAndUpdate(
            doc._id,
  ...
  describe('#delete', function() {
    it('should delete a binder by _id', function(done) {
      new Binder({ title: 'ToDelete' })
        .save()
        .then(function(doc) {
          return Binder.deleteOne({ _id: doc._id });
        })
